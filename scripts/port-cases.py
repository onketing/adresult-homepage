...
LEADS = {
    ...
    "167542522": "S피부과는 처음 3개월만 계약했다가 반년 가까이 이어지고 있는 병원입니다. 광고 한 달 만에 플레이스 유입이 1,000회 가까이 늘고 신환이 증가했으며, 꾸준한 노출 관리·콘텐츠 리뉴얼·빠른 피드백으로 실장님이 감사 선물을 보낼 만큼 신뢰를 쌓았습니다.",
    "167545669": "네트워크를 탈퇴하고 독립 개원한 통증치료 병원이, 개원 1달 전부터 애드리절트와 준비해 개원 3개월 만에 예약이 꽉 차 더 이상 환자를 받지 못할 정도로 자리 잡았습니다.",
    "167560320": "블로그 광고를 3년간 진행해 콘텐츠가 반복되던 병원에, 애드리절트가 먼저 전면 리뉴얼을 제안한 사례입니다. 종합병원 승격·새 진료과목 추가 시점에 맞춰 콘텐츠와 디자인을 새로 단장했습니다.",
    "167560631": "애드리절트는 고객사 병원의 리뷰와 채널을 매일 모니터링합니다. 악성 리뷰가 달린 병원에는 카카오맵 대응 팁을 정리해 전달하는 등, 단순히 악플만 확인하는 1차원 모니터링을 넘어 실질적인 고민을 함께 해결합니다.",
}
...
def img_host(src):
    """컨텐츠 이미지 호스트 판별. /upload/(imweb) 또는 네이버 postfiles 만 채택.
    카카오/OGQ 이모티콘 스티커(storep-phinf·ogq_)는 콘텐츠가 아니므로 제외."""
    if "storep-phinf" in src or "/ogq_" in src:
        return None
    if "/upload/" in src:
        return "upload"
